...
const EditResume = () => {
  ...
  const [resumeData, setResumeData] = useState({
    ...
    template: {
      theme: "01", // default to first template
      colorPalette: [],
    },
    contactInfo: {
      email: "",
  ...
  // UY[DATYE ARRAY ITMES STATE USING INDEX]
  ...
  // REMOVING THE INFO USING THE INDEX
  ...
  const fetchResumeDetailsById = async () => {
    if (!resumeId) {
      console.warn('No resumeId provided in route params');
    ...
    try {
      ...
      if (response.data && response.data.profileInfo) {
        ...
        console.warn('Resume response missing profileInfo, response:', response);
      }
    } catch (error) {
      console.error('Error fetching resume:', error);
      if (error.response && error.response.status === 404) {
        console.warn('Resume not found (404) for id:', resumeId);
        toast.error('Resume not found (404). You will be redirected.');
  ...
  // IT LL HELP IN CHOOSING THE PREVIEW AS WELL AS HELP IN DOWNLOADING THE RESUME ALSO SAVE THE RESUME AS A IMAGE
      ...
      <DashboardLayout>
        <div className={containerStyles.main}>
          <div className="p-12 text-center">
            ...
            <p className="text-slate-600 mb-6">The resume you're looking for does not exist or you don't have access to it. You will be redirected to the dashboard shortly.</p>
            <button className={buttonStyles.back} onClick={() => navigate('/dashboard')}>Go to Dashboard</button>
          </div>
        </div>
      </DashboardLayout>
    );
      ...
      <div className={containerStyles.main}>
        <div className={containerStyles.header}>
          ...
          <div style={{marginTop: '8px'}}>
            <span style={{fontWeight: 'bold', color: '#8b5cf6'}}>Selected Theme ID:</span>
            <span style={{marginLeft: '8px', color: '#333'}}>{resumeData?.template?.theme}</span>
          </div>
